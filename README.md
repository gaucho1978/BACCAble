...
Note2: The parameters array is customizable ,It resides in main.c. (search for uds_params_array array)
...
Note3: if you change number of elements in the array, you shall update total number of elements in the variable total_pages_in_dashboard_menu.
...
We are introducing automatic compilation on github. Stable releases are dawnloadable from RELEASE section (right side of this page).
...
The obtained elf file (downloaded from online release version, or locally compiled), can be flashed using stm32cubeProgrammer.

Flash procedure:
- press reset button on the board, then connect usb to pc (the board will be detected as serial device named "stm32 bootloader"
