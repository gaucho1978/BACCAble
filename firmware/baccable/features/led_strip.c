...
/* Prepare the enabled LED strip and turn it off after vehicle activity stops. */
    ...
    if (settings_state.led_strip_controller_enabled == 1) {
        ...
        if (display_state.leds_strip_is_on) { // if the strip is on,
            if (currentTime - comfort_state.time_since_last_received_accelerator_message >
                ...
                led_strip_shutdown();
                display_state.leds_strip_is_on =
                    0; // Mark the strip as off until new vehicle activity arrives.
            }
        }
    }
}
