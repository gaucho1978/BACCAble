...
void setLedsVumeter(uint8_t volume) {
    ...
    for (i = 0; i < 15; i++) {
        ...
        // distanzaDalSetpoint= 15 * 1/powf(distanzaDalSetpoint,3); // diventa una variabile k che si riduce
        // proporzionalmente alla distanza dalla soglia

        // usiamo questa variabile per aggiungere un poco di sfumatura sulla transizione
        distanzaDalSetpoint = myAbs(15 - volume - i); // oscilla tra 15 e 0
        if (distanzaDalSetpoint == 0)
            distanzaDalSetpoint = 22;
        if (distanzaDalSetpoint == 1)
            distanzaDalSetpoint = 3;
        if (distanzaDalSetpoint == 2)
            distanzaDalSetpoint = 2;
        if (distanzaDalSetpoint == 3)
            distanzaDalSetpoint = 1;
        if (distanzaDalSetpoint != 22 && distanzaDalSetpoint != 3 && distanzaDalSetpoint != 2 &&
            distanzaDalSetpoint != 1)
            distanzaDalSetpoint = 0;
    ...
    for (i = 31; i < 46; i++) {
        ...
        if (distanzaDalSetpoint == 3)
            distanzaDalSetpoint = 1;
        if (distanzaDalSetpoint != 15 && distanzaDalSetpoint != 2 && distanzaDalSetpoint != 1)
            distanzaDalSetpoint = 0;
...
// this function sets one led (my led strip is BRG (blue,red,green). you can set this function according to
// your led strip type.
...
// init Timer used to control the led strip by generating pwm output updating the duty cycle in order to
// implement the WS2812 protocol.
//  In my case I use Timer1, channel4 because my gpio is PA11 (used as usb pin in original canable firmware.
//  In order to use a Timer clock of 48Mhz I use internal HSI48 oscillator (48Mhz) and prescaler=1
//  (timerClock= APB_tim_clk/prescaler) For WS2811 and WS2812 leds strip We need a pwm period of 1,25us.
//  Considering that pwmPeriod= ARR/timerClok => ARR=pwmPeriod * timerClock = 1,25us * 48Mhz = 60
//  ARR is the AutoReloadRegister and defines when the timer counter shall restart from 0, therefore we set it
//  to 60 (always -1 for these parameters type), in order to obtain a pwm output period of 1,25usec. In WS2811
//  and WS2812 leds strip protocol, the logical 0 impplies a pwm duty cycle of around 28% (0,28), and logical
//  1 implies a duty cycle of around 56% (0,56). the dutyCycle= CCRx/ARR  => CCRx=dutyCycle * ARR CCR is the
//  CaptureCompareRegister and there is one for each channel. Since we use channel4, we shall set CCR4 as it
//  follows:
...
// DMA is used to feed the timer with an array of CCR values, in order to allow efficient code execution,
// without loosing time in our main loop to control the gpio realtime toggle
...
// This function is called when DMA completes to send its bits. When this occour we will stop the pwm
// generation, in order to be
//  ready to send a new frame, otherwise last bit continues to be sent generating a mistake in the ws281x
//  protocol output.
