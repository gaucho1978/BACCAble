    ...
    if (huart->Instance == USART2) {
    	...
    	if((rxBuffer[0]>=C1BusID) && (rxBuffer[0]<=BHBusIDAllSleepAck)){ //if the received char indicates the beginning of a message
			if(syncObtained){ //if we were sync, we can process the message, since the first char is correct and the sync indicates that te remaining part too is complete
				...
				HAL_UART_Receive_IT(&huart2, &rxBuffer[0], DASHBOARD_MESSAGE_MAX_LENGTH + 1); //receive next frame
			}else{ //otherwise we were not sync, therefore we need to receive the remaining part of the message
				syncObtained=1;
				HAL_UART_Receive_IT(&huart2, &rxBuffer[1], DASHBOARD_MESSAGE_MAX_LENGTH); //receive remaining part of the frame
			}
