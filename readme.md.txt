...
the can start&stop disabler function is done by simply shorting a gpio to ground to simulate button press on the car panel, with a delay after the device was switched on. The used resistor is switable for my car. each one shoud perform some checks on the panel with a multimeter to find the proper resistor value. Same approach shall be used on the can bus protocol, even if the accelertor pedal position should be the same on each car.
